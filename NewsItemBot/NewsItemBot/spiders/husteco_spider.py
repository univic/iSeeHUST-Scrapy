...
from NewsItemBot.items import NewsitembotItem
...
class HustecoSpider(scrapy.Spider):

    name = "husteco"

    def start_requests(self):
        urls = [
            'http://eco.hust.edu.cn/zsjy/zyfz/sxjyxx.htm',
        ]
    ...
    def parse(self, response):
        ...
        nodes = response.xpath('//div[@class="main_con"]//li')
        logging.info(f">>>>>>>>{response.request.url} : {len(nodes)}")

        # 查找类名为listma的div标签，解析栏目名称
        section_name = response.xpath('//div[@class="location"]/h3/text()').extract()[0]
        ...
        for node in nodes:
            ...
            item_loader.add_value("tags", ["经济学院", section_name])

            # 将str类型日期转换为Datetime类型
            date_str = node.xpath('i/text()').extract()[0]
